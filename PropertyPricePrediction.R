my_data=read.csv("C:/Users/shubhamv/Downloads/Property_Price_Train.csv")
dim(my_data)
names(my_data)
str(my_data)
...
median_data
sum(is.na(lot_extent_data)) # Missing values
lot_extent_data
...
sum(is.na(my_data$Lot_Size_data)) # missing values                                                   #   1300    7549    9477   10517   11603  215245 
...
class(my_data$Road_Type)
Road_type_data=as.factor(my_data$Road_Type)              # Gravel  Paved 
                                                           #  6   1453 
table(Road_type_data)                                     #   1     2
road_type_data_numeric=as.numeric(Road_type_data) # label encoding
# my_data$Road_Type <- ifelse(my_data$Road_Type=="Gravel",1,0) # one more type of encoding
...
#Lane_Type_data=replace(Lane_Type_data,na,"no alley access")
Lane_Type_data
str(my_data$Lane_Type_data)
...
my_data$Neighborhood
table(my_data$Neighborhood)
...
Overall_Material_data_numeric <- as.numeric(Overall_Material_data)
table(Overall_Material_data_numeric)
...
House_Condition=as.factor(House_Condition_data)
...
max(tabled_data_constyear)
                                                       ...
                                                       ####   max 67 houses in 2006
#______________________________________________________________________

Remodel_Year_data=my_data$Remodel_Year

table(Remodel_Year_data)
max(table(Remodel_Year_data))                         # max 178 houses are remodeled in 1950 
...
sum(is.na(my_data$Brick_Veneer_Type_data))
...
sum(is.na(Basement_Condition_data))
...
table(electrical_system_data_numeric)
##dim(dat)
...
par(mfrow=c(2,3))
boxplot(numerical_data[,c(1:5)],col="blue",main='1:5 VARIABLES')
boxplot(numerical_data[,c(6:10)],col="blue",main='6:10 VARIABLES')
boxplot(numerical_data[,c(11:15)],col="blue",main='11:15 VARIABLES')
boxplot(numerical_data[,c(16:20)],col="blue",main='16:20 VARIABLES')
boxplot(numerical_data[,c(21:25)],col="blue",main='21:25 VARIABLES')
boxplot(numerical_data[,c(26:28)],col="blue",main='26:28 VARIABLES')
...
hist(differ_error, col = "green")          ##normal
...
plot(df$id_data,differ_error, col='purple', main="Independence of erros")
